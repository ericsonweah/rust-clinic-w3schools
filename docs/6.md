Perfect 🚀 — here’s the **full corrected `main.rs`** with **middleware + `next()` pipeline**, **params**, **wildcards**, and **regex routes** all integrated.

This is now a true **Express-like RawExpress prototype** in Rust.

---

## 📄 `main.rs`

```rust
use std::collections::HashMap;
use std::io::{Read, Write};
use std::net::{TcpListener, TcpStream};
use std::sync::Arc;
use std::thread;
use regex::Regex;

//
// ---------- HttpRequest ----------
//
#[derive(Debug, Clone)]
pub struct HttpRequest {
    pub method: String,
    pub path: String,
    pub params: HashMap<String, String>,
}

//
// ---------- HttpResponse ----------
//
#[derive(Debug, Clone)]
pub struct HttpResponse {
    pub status: String,
    pub headers: HashMap<String, String>,
    pub body: String,
}

impl HttpResponse {
    pub fn ok(body: &str) -> Self {
        let mut headers = HashMap::new();
        headers.insert("Content-Type".into(), "text/plain".into());
        headers.insert("Content-Length".into(), body.len().to_string());
        Self {
            status: "HTTP/1.1 200 OK".into(),
            headers,
            body: body.into(),
        }
    }

    pub fn with_status(code: u16, body: &str, content_type: &str) -> Self {
        let mut headers = HashMap::new();
        headers.insert("Content-Type".into(), content_type.into());
        headers.insert("Content-Length".into(), body.len().to_string());
        Self {
            status: format!("HTTP/1.1 {} CUSTOM", code),
            headers,
            body: body.into(),
        }
    }

    pub fn to_string(&self) -> String {
        let mut resp = format!("{}\r\n", self.status);
        for (k, v) in &self.headers {
            resp.push_str(&format!("{}: {}\r\n", k, v));
        }
        resp.push_str("\r\n");
        resp.push_str(&self.body);
        resp
    }
}

//
// ---------- res module ----------
//
pub mod res {
    use super::HttpResponse;

    pub fn ok(body: &str) -> HttpResponse {
        HttpResponse::ok(body)
    }

    pub fn not_found(body: &str) -> HttpResponse {
        HttpResponse::with_status(404, body, "text/plain")
    }

    pub fn json(body: &str) -> HttpResponse {
        HttpResponse::with_status(200, body, "application/json")
    }
}

//
// ---------- Route matcher ----------
//
fn match_route(pattern: &str, path: &str) -> Option<HashMap<String, String>> {
    let pat_parts: Vec<&str> = pattern.trim_matches('/').split('/').collect();
    let path_parts: Vec<&str> = path.trim_matches('/').split('/').collect();

    let mut params = HashMap::new();

    let mut i = 0;
    while i < pat_parts.len() {
        let p = pat_parts[i];
        let a = path_parts.get(i).unwrap_or(&"");

        if p == "*" {
            // Wildcard: capture rest of the path
            let rest = path_parts[i..].join("/");
            params.insert("wildcard".to_string(), rest);
            return Some(params);
        } else if p.starts_with(':') {
            // Named param, maybe regex
            if let Some(open) = p.find('(') {
                let name = &p[1..open];
                let regex_pat = &p[open..];
                let re = Regex::new(regex_pat).ok()?;
                if re.is_match(a) {
                    params.insert(name.to_string(), a.to_string());
                } else {
                    return None;
                }
            } else {
                let name = &p[1..];
                params.insert(name.to_string(), a.to_string());
            }
        } else if p != *a {
            return None;
        }
        i += 1;
    }

    if pat_parts.len() != path_parts.len() && !pat_parts.contains(&"*") {
        return None;
    }

    Some(params)
}

//
// ---------- Middleware types ----------
//
type Next<'a> = &'a dyn Fn(HttpRequest) -> HttpResponse;
type Middleware = Box<dyn Fn(HttpRequest, Next) -> HttpResponse + Send + Sync>;

//
// ---------- App ----------
//
pub struct App {
    routes: Arc<Vec<(String, String, Box<dyn Fn(HttpRequest) -> HttpResponse + Send + Sync>)>>,
    middlewares: Arc<Vec<Middleware>>,
}

impl App {
    pub fn new() -> Self {
        Self {
            routes: Arc::new(Vec::new()),
            middlewares: Arc::new(Vec::new()),
        }
    }

    pub fn get<F>(&mut self, path: &str, handler: F)
    where
        F: Fn(HttpRequest) -> HttpResponse + Send + Sync + 'static,
    {
        Arc::get_mut(&mut self.routes)
            .unwrap()
            .push(("GET".into(), path.into(), Box::new(handler)));
    }

    pub fn use_middleware<F>(&mut self, f: F)
    where
        F: Fn(HttpRequest, Next) -> HttpResponse + Send + Sync + 'static,
    {
        Arc::get_mut(&mut self.middlewares).unwrap().push(Box::new(f));
    }

    pub fn listen(self, addr: &str) -> std::io::Result<()> {
        let listener = TcpListener::bind(addr)?;
        println!("Server running at http://{}", addr);

        for stream in listener.incoming() {
            let stream = stream?;
            let routes = self.routes.clone();
            let mws = self.middlewares.clone();
            thread::spawn(move || {
                handle_connection(stream, routes, mws);
            });
        }
        Ok(())
    }
}

//
// ---------- Middleware runner ----------
//
fn run_pipeline(
    req: HttpRequest,
    mws: &[Middleware],
    routes: &[(String, String, Box<dyn Fn(HttpRequest) -> HttpResponse + Send + Sync>)],
) -> HttpResponse {
    if let Some((first, rest)) = mws.split_first() {
        let next: Next = &|req2: HttpRequest| run_pipeline(req2, rest, routes);
        first(req, next)
    } else {
        // Route matching
        for (m, pattern, handler) in routes.iter() {
            if &req.method == m {
                if let Some(params) = match_route(pattern, &req.path) {
                    let mut new_req = req.clone();
                    new_req.params = params;
                    return handler(new_req);
                }
            }
        }
        res::not_found("404 Not Found")
    }
}

//
// ---------- Connection Handler ----------
//
fn handle_connection(
    mut stream: TcpStream,
    routes: Arc<Vec<(String, String, Box<dyn Fn(HttpRequest) -> HttpResponse + Send + Sync>)>>,
    middlewares: Arc<Vec<Middleware>>,
) {
    let mut buffer = [0; 512];
    let _ = stream.read(&mut buffer);

    let req_str = String::from_utf8_lossy(&buffer);
    let mut lines = req_str.lines();
    let request_line = lines.next().unwrap_or("");
    let parts: Vec<&str> = request_line.split_whitespace().collect();

    let response = if parts.len() >= 2 {
        let method = parts[0].to_string();
        let path = parts[1].to_string();
        let req = HttpRequest {
            method,
            path,
            params: HashMap::new(),
        };
        run_pipeline(req, &middlewares, &routes)
    } else {
        res::not_found("400 Bad Request")
    };

    let resp_str = response.to_string();
    let _ = stream.write_all(resp_str.as_bytes());
    let _ = stream.flush();
}

//
// ---------- main ----------
//
fn main() {
    let mut app = App::new();

    // Middleware 1: logger
    app.use_middleware(|req, next| {
        println!("[LOG] {} {}", req.method, req.path);
        next(req)
    });

    // Middleware 2: block access
    app.use_middleware(|req, next| {
        if req.path == "/blocked" {
            res::not_found("This path is blocked")
        } else {
            next(req)
        }
    });

    // Routes
    app.get("/", |_req| {
        res::ok("Welcome to RawExpress with middleware!")
    });

    app.get("/users/:id", |req| {
        res::ok(&format!("User ID: {}", req.params["id"]))
    });

    app.get("/files/*", |req| {
        res::ok(&format!("File path: {}", req.params["wildcard"]))
    });

    app.get("/posts/:id([0-9]+)", |req| {
        res::ok(&format!("Post ID: {}", req.params["id"]))
    });

    app.listen("127.0.0.1:3000").unwrap();
}
```

---

## 🚦 Run & Test

```bash
cargo run
```

### Middleware logger

```bash
curl http://127.0.0.1:3000/
```

➡ Logs `[LOG] GET /`

### Middleware blocking

```bash
curl http://127.0.0.1:3000/blocked
```

➡ `This path is blocked`

### Route params

```bash
curl http://127.0.0.1:3000/users/42
```

➡ `User ID: 42`

### Wildcard

```bash
curl http://127.0.0.1:3000/files/images/icons/logo.png
```

➡ `File path: images/icons/logo.png`

### Regex route

```bash
curl http://127.0.0.1:3000/posts/123
```

➡ `Post ID: 123`

```bash
curl http://127.0.0.1:3000/posts/abc
```

➡ `404 Not Found`

---

✅ You now have:

* Middleware with `next()`.
* Multiple middleware chaining.
* Route params, wildcards, regex.
* Express-like DX in Rust.

---

👉 Do you want me to show you how to add **response chaining** (e.g. `res::ok("Hello").with_status(201).set_header("X-Powered-By", "RawExpress")`) so you get an even closer feel to Express?
