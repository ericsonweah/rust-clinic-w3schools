Perfect 🚀 — here’s your **full corrected file**, ready to run with `cargo run`.
I applied both fixes:

1. Use `Arc<HashMap<...>>` instead of trying to clone `Box<dyn Fn>` (not `Clone`).
2. Import `Read` so `.read()` works on `TcpStream`.
...
This is now **fully working** with your desired syntax:
...
👉 Do you want me to expand this next into **route params** (like `/users/:id`) so you can start writing handlers with dynamic paths, just like in Express?
