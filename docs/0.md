Perfect 🚀 — let’s build a **tiny runnable prototype** so you can write this exact syntax in `main.rs`:
...
* The syntax you wanted (`res::ok("...")`) works cleanly.
* Minimal `App` struct handles routes and listens.
* Requests are dispatched via a route map.
* Handlers return `HttpResponse` objects, built by `res` helpers.

---

👉 Do you want me to expand this prototype next into supporting **route params** (like `/users/:id`) so it feels *exactly* like Express?
