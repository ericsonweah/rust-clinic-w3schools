...
* ✅ Middleware pipeline with `next(req).await`
* ✅ Route params (`:id`), wildcards (`*`), regex (`:id([0-9]+)`)
* ✅ Response chaining (`.status().set_header().body()`)
* ✅ Body parsing (raw, form-urlencoded, JSON)
* ✅ Async/await handlers and middleware
...
async fn run_pipeline(
    ...
    if let Some((first, rest)) = mws.split_first() {
        let next: Next = &|req2: HttpRequest| run_pipeline(req2, rest, routes).boxed();
...
### Async middleware logging

```bash
curl http://127.0.0.1:3000/
```

➡ Logs `[LOG] GET /` with ~50ms delay.

### Async route with DB-like delay

```bash
curl http://127.0.0.1:3000/users/42
```

➡ Waits ~100ms, returns `Fetched user 42`.
...
✅ Now RawExpress supports **async/await middleware + handlers**, body parsing, chaining, and routing.

👉 Do you want me to show you **how to replace `block_on` with your own zero-dep RawExecutor** (so you’re not tied to `futures`/`async-std`), making it fully RawNode-style?
