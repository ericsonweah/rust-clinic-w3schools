Perfect 🚀 — here’s the **full integrated `main.rs`** with:

* ✅ Middleware + `next()` pipeline
* ✅ Route params (`:id`), wildcards (`*`), regex routes (`:id([0-9]+)`)
* ✅ Builder-style chaining (`.status(...).set_header(...).body(...)`)
...
fn main() {
    ...
    app.get("/", |_req| {
        res::ok("Welcome")
            ...
            .body("Welcome to RawExpress with chaining + middleware!")
    });
...
➡ Logs `[LOG] GET /`
➡ Response includes `X-Powered-By: RawExpress` header
...
➡ `File path: images/icons/logo.png`
...
✅ Now you have an **Express-class Rust framework prototype** with:

* Middleware pipeline with `next()`.
* Route params, wildcards, regex.
* Response chaining for headers/status/body.

---

👉 Do you want me to show you **how to add POST body parsing (JSON, form-urlencoded)** next, so you can handle APIs like `app.post("/login", ...)` with `req.body`?
